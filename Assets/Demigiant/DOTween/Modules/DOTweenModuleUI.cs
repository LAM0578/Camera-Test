...
namespace DG.Tweening
{
    public static class DOTweenModuleUI
    {
        ...
        public static Tweener DOAnchorPosX(this RectTransform target, float endValue, float duration, bool snapping = false)
        {
            if (duration <= 0)
            {
                target.anchoredPosition = new Vector2(endValue, target.anchoredPosition.y);
                return null;
            }
        ...
        public static Tweener DOAnchorPosY(this RectTransform target, float endValue, float duration, bool snapping = false)
        {
            if (duration <= 0)
            {
                target.anchoredPosition = new Vector2(target.anchoredPosition.x, endValue);
                return null;
            }
        ...
        public static Tweener DOFloat(this float target, float endValue, float duration)
        {
            return DOTween.To(() => target, x => target = x, endValue, duration).SetTarget(target);
        } 

        public static Tweener DOFloat(this Material target, float endValue, float duration, string propertyName = "_Alpha")
        {
            return DOTween.To(() => target.GetFloat(propertyName), x => target.SetFloat(propertyName, x), endValue, duration).SetTarget(target);
        }
        ...
        public static Tweener DOReferRes(this CanvasScaler target, Vector2 endValue, float duration, bool snapping = false)
        {
            Vector2 resIn = target.referenceResolution;
            return DOTween.To(() => new Vector2(resIn.x, resIn.y),
                x =>
                {
                    target.referenceResolution = new Vector2(x.x, x.y);
                }, endValue, duration)
                .SetOptions(snapping).SetTarget(target);
        }
        ...
        public static Tweener DOShadowColor(this Shadow target, Color endValue, float duration)
        {
            return DOTween.To(() => target.effectColor, x => target.effectColor = x, endValue, duration).SetTarget(target);
        }
        ...
        public static Tweener DOOutlineColor(this Outline target, Color endValue, float duration)
        {
            return DOTween.To(() => target.effectColor, x => target.effectColor = x, endValue, duration).SetTarget(target);
        }
